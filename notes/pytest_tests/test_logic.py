...
def test_other_user_cant_edie_note(not_author_client, form_data, note):
    url = reverse('notes:edit', kwargs={'slug': note.slug})
    response = not_author_client.post(url, form_data)
    assert response.status_code == HTTPStatus.NOT_FOUND
    note_form_db = Note.objects.get(id=note.id)
    assert note.title == note_form_db.title
    assert note.text == note_form_db.text
    assert note.slug == note_form_db.slug
